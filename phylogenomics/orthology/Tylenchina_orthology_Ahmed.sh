...
# Removing orthologs with represented in less than 36 taxa from the proteinortho output
awk '/^#/ {header=$0; print $0}; $1 > 35' ../02_PRTNRTH/orthology_0.00001.proteinortho.tsv > ../02_PRTNRTH/tylenchina_reduced.tsv

# Using protein_grab_proteins.pl script to extract only orthologs
proteinortho_grab_proteins.pl -t -exact ../02_PRTNRTH/tylenchina_reduced.tsv ../01_INPUT/proteins/*.faa


# aligning fasta files
for FILENAME in *.fasta
do
	mafft --auto --maxiterate 1000  --thread 3 $FILENAME > ../04_MAFFT/$FILENAME
done
...
cd ../07_IQTREE
find . -name "*.fa" | xargs rename 's/orthology0.00001.proteinorthoreduced.tsv.//g'
...
for FILENAME in *.fa
do
iqtree2 -s $FILENAME -st AA -m MFP -msub nuclear -B 1000 -T 3
done


# removing paralogs using phylopypruner
Phylopypruner --dir ./ --min-len 50 --trim-lb 5 --min-support 0.75 --prune MI --mask pdist --min-taxa 36 --outgroup PleSam2
...
sed 's/AUTO/LG/g' partition_data.txt > partition_data_tylenchina.txt


raxmlHPC-PTHREADS-SSE3 -s supermatrix.fas -n tylenchina -m PROTGAMMALG -o PleSam2 -p 12345 -q partition_data_tylenchina.txt -T 3 --asc-corr=lewis 2>&1 | tee logfile.txt

cp partition_data_tylenchina.txt ../10_IQTREE/
cp supermatrix.fas ../10_IQTREE/
cp RAxML_bestTree.tylenchina ../10_IQTREE/

cd ../10_IQTREE/

iqtree2 -nt 3 -bb 1000 -bnni -safe -st AA -s supermatrix.fas -m C60+F+G -fmax -seed 12345 -ft RAxML_bestTree.tylenchina -o PleSam2 --prefix output -q partition_data_tylenchina.txt 2>&1 | tee logfile.txt
...
iqtree2 -s $FILENAME -st AA -m MFP -msub nuclear -bb 1000 -nt 3
done
...
find ./ -name "*_BS75.fa.treefile" | xargs gsed 's/[|_].\{5,7\}:/:/g' >> tylenchina_gene.tre

java -jar -Xmx12000M -D"java.library.path=lib/" ~/Astral/astral.5.7.3.jar -i tylenchina_gene.tre -o tylenchina_consensus.tre 2>&1 |tee tylenchina.log
...
find ./ -name "*_pruned.fa.treefile" | xargs gsed 's/_.\{5,7\}:/:/g' >> tylenchina_gene.tre


java -jar -Xmx12000M -D"java.library.path=lib/" ~/Astral/astral.5.7.3.jar -i tylenchina_gene.tre -o tylenchina_consensus.tre 2> tylenchina.log

phylo_rename tylenchina_consensus.tre
