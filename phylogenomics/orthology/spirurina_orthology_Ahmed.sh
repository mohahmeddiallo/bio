...
# Removing orthologs with represented in less than 23 taxa from the proteinortho output
awk '/^#/ {header=$0; print $0}; $1 > 22' ../02_PRTNRTH/orthology_0.00001.proteinortho.tsv > ../02_PRTNRTH/spirurina_reduced.tsv

# Using protein_grab_proteins.pl script to extract only orthologs
proteinortho_grab_proteins.pl -t -exact ../02_PRTNRTH/spirurina_reduced.tsv ../01_INPUT/proteins/*.faa
...
cd ../07_IQTREE
find . -name "*.fa" | xargs rename 's/spirurinareduced.tsv.//g'
echo Done
echo
...
Phylopypruner --dir ./ --min-len 50 --trim-lb 5 --min-support 0.75 --prune MI --mask pdist --min-taxa 23 --outgroup PleSam2
...
sed 's/AUTO/LG/g' partition_data.txt > partition_data_spirurina.txt


raxmlHPC-PTHREADS-SSE3 -s supermatrix.fas -n spirurina -m PROTGAMMALG -o PleSam2 -p 12345 - partition_data_spirurina.txt -T 8 --asc-corr=lewis 2>&1 | tee logfile.txt

cp partition_data_spirurina.txt ../10_IQTREE/
cp supermatrix.fas ../10_IQTREE/
cp RAxML_bestTree.spirurina ../10_IQTREE/

cd ../10_IQTREE/

iqtree2 -nt 10 -bb 1000 -bnni -safe -st AA -s supermatrix.fas -m C60+F+G -fmax -seed 1234 -ft RAxML_bestTree.spirurina -o PleSam2 --prefix output -q partition_data_spirurina.txt 2>&1 | tee logfile.txt
...
find ./ -name "*_BS75.fa.treefile" | xargs gsed 's/_.\{5,7\}:/:/g' >> spirurina_gene.tre


java -jar -Xmx12000M -D"java.library.path=lib/" ~/Astral/astral.5.7.3.jar -i spirurina_gene.tre -o spirurina_consensus.tre 2>&1 |tee spirurina.log


phylo_rename spirurina_consensus.tre
