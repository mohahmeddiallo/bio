...
# Removing orthologs with represented in less than 13 taxa from the proteinortho output
awk '/^#/ {header=$0; print $0}; $1 > 12' ../02_PRTNRTH/orthology_0.00001.proteinortho.tsv > ../02_PRTNRTH/Trichinellida_reduced.tsv

# Using protein_grab_proteins.pl script to extract only orthologs
proteinortho_grab_proteins.pl -t -exact ../02_PRTNRTH/Trichinellida_reduced.tsv ../01_INPUT/proteins/*.faa
...
cd ../07_IQTREE
find . -name "*.fa" | xargs rename 's/Trichinellidareduced.tsv.//g'
echo Done
echo
...
Phylopypruner --dir ./ --min-len 50 --trim-lb 5 --min-support 0.75 --prune MI --mask pdist --min-taxa 23 --outgroup LonElo8
...
sed 's/AUTO/LG/g' partition_data.txt > partition_data_trichinellida.txt


raxmlHPC-PTHREADS-SSE3 -s supermatrix.fas -n trichinellida -m PROTGAMMALG -o LonElo8 -p 12345 -q partition_data_trichinellida.txt -T 8 --asc-corr=lewis 2>&1 | tee logfile.txt

cp partition_data_trichinellida.txt ../10_IQTREE/
cp supermatrix.fas ../10_IQTREE/
cp RAxML_bestTree.trichinellida ../10_IQTREE/

cd ../10_IQTREE/

iqtree2 -nt 10 -bb 1000 -bnni -safe -st AA -s supermatrix.fas -m C60+F+G -fmax -seed 12345 -ft RAxML_bestTree.trichinellida -o LonElo8 --prefix output -q partition_data_trichinellida.txt 2>&1 | tee logfile.txt

find ../08_PHPPRNR/TRICHINELLIDA_phylopypruner_output/output_alignments/ -name "*fa" -exec cp {} ../13_IQTREE \;
...
find ./ -name "*_BS75.fa.treefile" | xargs gsed 's/[|_].\{5,7\}:/:/g' >> trichinellida_gene.tre

java -jar -Xmx12000M -D"java.library.path=lib/" ~/Astral/astral.5.7.3.jar -i trichinellida_gene.tre -o trichinellida_consensus.tre 2>&1 |tee spirurina.log
